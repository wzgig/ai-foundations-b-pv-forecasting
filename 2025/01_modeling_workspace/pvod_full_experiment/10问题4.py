...
def run_input_mode_experiments(df, model_dict, modes=['nwp', 'lmd', 'mixed']):
    ...
    def get_feature_columns(mode):
        if mode == 'nwp':
            return ['nwp_globalirrad', 'nwp_directirrad', 'nwp_temperature',
                    'nwp_humidity', 'nwp_windspeed', 'nwp_pressure', 'wind_x', 'wind_y']
        elif mode == 'lmd':
            return ['lmd_totalirrad', 'lmd_diffuseirrad', 'lmd_temperature',
    ...
    for mode in modes:
        ...
        for name, model in model_dict.items():
            ...
            if name == "BiFusionModel":
                model_instance = BiFusionModel(input_len=96, input_dim=input_dim)
            elif name == "PureLSTM":
                model_instance = PureLSTM(input_len=96, input_dim=input_dim)
...
def visualize_q4_results(df_results):
    import matplotlib.cm as cm

    plt.rcParams['font.sans-serif'] = ['SimHei']
    plt.rcParams['axes.unicode_minus'] = False

    models = sorted({idx.split('_')[0] for idx in df_results.index})
    modes = sorted({idx.split('_')[1] for idx in df_results.index})
    metrics = ['RMSE', 'MAE', 'MAPE', 'C_R', 'Q_R', 'E_rmse']
    ...
    for model in models:
        ...
        for i, mode in enumerate(modes):
            key = f"{model}_{mode}"
            values = df_results.loc[key, metrics].tolist()
            values += values[:1]
            ax.plot(angles, values, label=mode.upper(), linewidth=2)
    ...
    for metric in ['RMSE', 'MAPE', 'E_rmse']:
        ...
        for i, mode in enumerate(modes):
            data = [df_results.loc[f"{model}_{mode}", metric] for model in models]
            plt.bar([x + i * 0.2 for x in range(len(models))], data, width=0.2, label=mode.upper())
        plt.xticks([x + 0.2 for x in range(len(models))], models)
        plt.title(f"{metric} 指标对比（不同输入模式）")
        plt.ylabel(metric)
        plt.legend()
        plt.tight_layout()
        plt.show()
    ...
    for metric in ['C_R', 'Q_R']:
        ...
        for model in models:
            y = [df_results.loc[f"{model}_{mode}", metric] for mode in modes]
            x = [input_dims[mode] for mode in modes]
            plt.plot(x, y, marker='o', label=model)
        plt.title(f"{metric} vs 输入特征维度")
        plt.xlabel("输入特征数量")
        plt.ylabel(metric)
        plt.grid(True)
        plt.legend()
...
if __name__ == "__main__":
    model_dict = {
        # "BiFusionModel": BiFusionModel(input_len=96, input_dim=15),
        # "PureLSTM": PureLSTM(input_len=96, input_dim=15),
        "FusionModel": FusionModel(input_len=96)
    }

    df_results = run_input_mode_experiments(df, model_dict)
    visualize_q4_results(df_results)
