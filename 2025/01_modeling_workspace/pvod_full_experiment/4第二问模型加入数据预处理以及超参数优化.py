...
# 融合 CEEMDAN+FFT+LDA 的特征提取代码块，需加入数据构建流程上方
...
# --- CEEMDAN 分解 ---
def apply_ceemdan(seq, max_imfs=5):
    ceemdan = CEEMDAN()
    ceemdan.noise_seed = 42  # 正确设置方式
    imfs = ceemdan(seq)
    return imfs[:max_imfs]


# --- 从单个 IMF 提取频域特征 ---
def extract_fft_features(imf, fs=96):
    fft_vals = np.abs(fft(imf))
    fft_freqs = fftfreq(len(imf), d=1/fs)
    fft_vals = fft_vals[:len(imf)//2]
    fft_freqs = fft_freqs[:len(imf)//2]

    power = np.sum(fft_vals ** 2)
    dominant_freq = fft_freqs[np.argmax(fft_vals)]
    spectral_entropy = entropy(fft_vals / (np.sum(fft_vals) + 1e-8))

    return [power, dominant_freq, spectral_entropy]

# --- 对一个序列提取全部 IMF 的频域特征 ---
def extract_ceemdan_hi_features(seq, max_imfs=5):
    imfs = apply_ceemdan(seq, max_imfs)
    all_features = []
    for imf in imfs:
        features = extract_fft_features(imf)
        all_features.extend(features)
    return all_features  # 每个 IMF 提取 [能量, 主频, 谱熵]

# --- 全数据集提取特征矩阵 ---
def transform_dataset_with_ceemdan_hi(X_raw, max_imfs=5):
    return np.array([extract_ceemdan_hi_features(seq, max_imfs) for seq in X_raw])

# --- 可选：LDA 降维（若后续需要标签区分天气类型） ---
def apply_lda_projection(X_feats, Y_labels, n_components=5):
    lda = LDA(n_components=n_components)
    return lda.fit_transform(X_feats, Y_labels)

# --- 替换 train_X 与 test_X ---
ceemdan_train_X = transform_dataset_with_ceemdan_hi(train_X, max_imfs=4)
ceemdan_test_X = transform_dataset_with_ceemdan_hi(test_X, max_imfs=4)

# 使用新的特征作为模型输入
train_X = ceemdan_train_X
test_X = ceemdan_test_X

# ======== 新增reshape操作 ========
# 重塑为 (样本数, 时间步长=96, 特征数=12) 的三维张量
# train_X = ceemdan_train_X.reshape(-1, 96, 12)
# test_X = ceemdan_test_X.reshape(-1, 96, 12)

import torch
import torch.nn as nn

class LSTMBranch(nn.Module):
    def __init__(self, input_len, input_dim, hidden_dim=64, num_layers=2):
        super().__init__()
    ...
    def forward(self, x):
        ...
        return self.fc(out[:, -1, :])  # 只用最后一时刻特征
...
class TCNBranch(nn.Module):
    def __init__(self, input_len, input_dim):
        super().__init__()
    ...
    def forward(self, x):
        x = x.permute(0, 2, 1)  # [batch, channels=input_dim, seq_len]
        x = self.net(x)
        return self.fc(x.mean(dim=2))  # 平均池化后全连接

class MLPBranch(nn.Module):
    def __init__(self, input_len, input_dim):
        super().__init__()
        self.flat_dim = input_len * input_dim
    ...
    def forward(self, x):
        x = x.view(x.size(0), -1)  # [batch, 96 * input_dim]
        return self.fc(x)

class FusionModel(nn.Module):
    def __init__(self, input_len=96, input_dim=12):
        super().__init__()
    ...
    def forward(self, x):
        ...
        concat = torch.cat([lstm_out, tcn_out, mlp_out], dim=1)  # [batch, 3*seq_len]
...
if __name__ == "__main__":
    # 原训练评估流程保持不变...
    # model = FusionModel(input_len=96)
    model = FusionModel(input_len=96, input_dim=12)  # 若你的 IMF + FFT 特征是 12维
    model, max_power = train_model(model, train_X, train_Y)
    preds, rmse, mae, mape = evaluate_model(model, test_X, test_Y, test_timestamps, df, max_power)
    export_prediction_table(preds, test_Y, test_timestamps, df)
    metrics = compute_all_metrics(test_Y, preds, test_timestamps, df, capacity_kW=6600)

    # 可视化调用
    visualize_predictions(preds, test_Y, test_timestamps, df)  # 原基础可视化
    ...
    # 交互式可视化（需要Jupyter环境或独立HTML）
    interactive_forecast_plot(preds, test_Y, test_timestamps, df)
