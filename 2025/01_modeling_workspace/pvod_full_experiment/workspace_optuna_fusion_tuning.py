...
# 优化后的融合模型与训练流程，加入 Dropout、AdamW、LR 调度器、Optuna
...
# ============ 模型结构增加 Dropout（建议替换原模型定义） ============
class LSTMBranch(nn.Module):
    def __init__(self, input_len, hidden_dim=64, num_layers=2, dropout=0.3):
        super().__init__()
...
class TCNBranch(nn.Module):
    def __init__(self, input_len, dropout=0.3):
        super().__init__()
        self.net = nn.Sequential(
            TCNBlock(1, 16, dilation=1, dropout=dropout),
            TCNBlock(16, 32, dilation=2, dropout=dropout),
...
class OptimizedFusionModel(nn.Module):
    def __init__(self, input_len=96, dropout=0.3):
        super().__init__()
        self.lstm_branch = LSTMBranch(input_len, dropout=dropout)
        self.tcn_branch = TCNBranch(input_len, dropout=dropout)
        self.mlp_branch = MLPBranch(input_len, dropout=dropout)
...
max_power = train_X_sub.max()  # 仅基于训练集计算
...
def objective(trial):
    ...
    model = OptimizedFusionModel(input_len=96, dropout=dropout)
    model.lstm_branch.lstm = nn.LSTM(1, hidden_dim, 2, batch_first=True, dropout=dropout)
    ...
    # 使用验证集的 RMSE 作为目标
    preds, rmse, _, _ = evaluate_model(model, test_X, test_Y, test_timestamps, df, max_power=1.0)
    return rmse
# ----------------------- 预测与评估 -----------------------
...
def export_prediction_table(preds, test_Y, test_timestamps, df, method_name='融合模型预测',
...
if __name__ == "__main__":
    ...
    model = OptimizedFusionModel(input_len=96)
    model, max_power = train_model(model, train_X, train_Y)
    preds, rmse, mae, mape = evaluate_model(model, test_X, test_Y, test_timestamps, df, max_power)
