...
if __name__ == "__main__":
    model_dict = {
        'LSTM': PureLSTM(input_len=96),
        'Fusion': FusionModel(input_len=96),
        'BiFusion': BiFusionModel(input_len=96)
    }
    ...
    for name, model in model_dict.items():
        print(f"\n================== 训练模型: {name} ==================")
        trained_model, max_power = train_model(model, train_X, train_Y)
        preds, *_ = evaluate_model(trained_model, test_X, test_Y, test_timestamps, df, max_power)
    ...
    # 输出结果表格
    df_metrics = pd.DataFrame(all_metrics).T.round(4)
    print("\n=== 三模型指标对比 ===")
    print(df_metrics)

    sns.heatmap(df_metrics, annot=True, cmap="YlGnBu")
    plt.title("模型评估指标对比 (白昼)")
    plt.tight_layout()
    plt.show()

    # 每日预测对比可视化（以 sample_index 为例）
    sample_index = 0
    ts = test_timestamps[sample_index]
    mask = (df['date_time'] >= ts) & (df['date_time'] < ts + pd.Timedelta(days=1))
    time_axis = df.loc[mask, 'date_time'].values[:96][df.loc[mask, 'is_daytime'].values[:96]]
    true = test_Y[sample_index][df.loc[mask, 'is_daytime'].values[:96]]

    plt.figure(figsize=(14, 6))
    plt.plot(time_axis, true, label="实际", linewidth=2)
    for name in all_preds:
        plt.plot(time_axis, all_preds[name][sample_index][df.loc[mask, 'is_daytime'].values[:96]], label=name, linestyle='--')
    plt.title(f"每日预测对比图（{ts.date()}）")
    plt.xlabel("时间")
    plt.ylabel("功率 (MW)")
    plt.legend()
    plt.grid(True)
    ...
    plt.figure(figsize=(12, 5))
    for name in all_preds:
        errors = all_preds[name] - test_Y
        plt.hist(errors.flatten(), bins=50, alpha=0.5, label=name)
    plt.title("残差分布对比")
    plt.xlabel("误差 (MW)")
    plt.ylabel("频数")
    plt.legend()
    ...
    # 如需导出 CSV
    df_metrics.to_csv("三模型白昼指标对比.csv", encoding="utf-8-sig")
