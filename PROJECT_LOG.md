...
- 重新对齐《人工智能基础B》期末大作业通知：一个压缩包提交，命名按“专业+班级+第一作者学号+姓名+作品名称”，材料包含项目主报告、项目代码包、演示视频和补充材料。
- 将已完成的 `人工智能基础B_王子成组.pdf` 与演示视频整理成可直接上传的正式提交目录。
- 补全通知中提到的“补充材料”模块，用模型权重、数据说明、核心图表、指标表、运行证据和创新点说明辅助评分。

### 主要改动

- 更新 `2025/06_最终提交文件夹/`，形成四个主目录：`01_项目主报告`、`02_演示视频`、`03_项目代码包`、`04_补充材料`，并新增根目录 `README_提交说明.md`。
- 将原始 00:03:52 视频裁剪为 00:02:58 的提交版 `人工智能基础B项目视频.mp4`，保留 1080P 画面和原音频，降低超过 3 分钟要求的风险。
- 补充材料区新增评分辅助说明、核心图表、指标与预测表、模型权重、运行证据、数据与引用六类材料。
- 修复 Q4 局地校正融合指标表：从已有三份 Q4 预测 CSV 重新汇总 `FusionModel_nwp`、`FusionModel_lmd`、`FusionModel_mixed` 三输入指标，恢复三输入对比。
- 更新 `2025/tools/project_health_check.py` 和 `tests/test_project_health.py`，让健康检查与单元测试忽略 `2025/06_最终提交文件夹` 生成物，避免提交包内代码副本影响源代码验证。
...
- `python 2025\tools\project_health_check.py`：通过，无 Python 解析错误，无受管输出问题。
- `python -m unittest discover -s tests -q`：通过，25 项测试 OK。
- `python 2025\run_project.py --list`：可列出五条工程链路。
- `python 2025\run_project.py --show all`：可读取全部摘要、指标和预测表。
- 最终提交文件夹约 144.19MB；同名 ZIP 约 114.7MB，低于 200MB 限制。
- 最终 ZIP：`2025/06_最终提交文件夹/电气工程及其自动化+2307+202310080241+王子成+光伏电站日前功率预测与运行工作台.zip`。
# 项目工作日志
...
- 按用户要求梳理课程提交要求中的 3 分钟演示视频录制方案。
- 明确视频中应展示哪些代码文件、如何展示环境搭建和一键运行、如何体现完整运行链路、每一步对应的功能和结果是什么。

### 主要改动

- 重写 `2025/05_delivery/演示视频脚本.md`，升级为“录制指南 + 分镜 + 旁白 + 代码文件对应表”。
- 新增 3 分钟时间轴，覆盖 Pages/项目目录、环境搭建、一键启动、工作台首页、运行结果、代码与命令、训练控制、运行解读和最终材料。
- 新增必须展示的核心代码文件清单，包括 `requirements.txt`、`run.bat`、`start_software.vbs`、`software_launcher.py`、`app.py`、`run_project.py`、各工程链路脚本、`llm/` 模块和 `docs/index.html`。
- 明确本项目没有外部网页爬取模块，视频中应表述为读取本地站点功率、NWP、LMD 数据，并从 `outputs/` 中抓取指标、图表和运行摘要展示。
- 补充可直接照读的完整旁白和录制禁忌，避免出现密钥、误称爬虫、现场完整重训或题号式叙事。

### 验证结果

- 文档为 Markdown 纯文本，已做结构和路径检查。

## 2026-07-02 项目标题统一为工程化名称

### 调整目标

- 按用户要求为当前项目文件夹和项目展示取一个正式标题，替换原目录名中的 `标题` 占位口径。
- 标题统一为 `光伏电站日前功率预测与运行工作台`，完整目录/提交包命名口径为 `电气工程及其自动化+2307+202310080241+王子成+光伏电站日前功率预测与运行工作台`。

### 主要改动

- 更新 `docs/index.html` 的浏览器标题和首屏 H1。
- 更新 `2025/app.py` 的 Streamlit 页面标题和首屏 H1。
- 更新 `2025/RUN_GUIDE.md` 中的本地目录路径示例。
- 更新 `2025/05_delivery/` 中的项目主报告 Markdown、团队分工、测试表、网站使用指南、演示视频脚本和最终提交包清单。
- 更新 `2025/tools/generate_csust_report.py` 的中英文报告标题，并重新导出：
  - `2025/05_delivery/项目主报告_终稿.docx`
  - `2025/05_delivery/项目主报告_终稿.pdf`
- 更新测试中的标题断言，确保 Pages 和报告继续使用新标题。
- 修正当前运行摘要中记录的旧绝对路径，使其与新目录口径一致。

### 验证结果

- `powershell -ExecutionPolicy Bypass -File 2025\tools\export_csust_report.ps1`：通过，DOCX/PDF 已重新生成；命令末尾有 Python 环境 `<prefix>` 提示，但退出码为 0。
- DOCX 文本抽查：包含新标题 `光伏电站日前功率预测与运行工作台`，不再包含上一版项目标题。
- PDF 文本抽查：可检索到新标题。

### 后续注意事项

- 物理外层文件夹重命名曾尝试执行，但 Windows 返回 `Access is denied`。当前 VS Code/Codex 会话正在占用该目录，不能强制结束用户编辑器。关闭当前 VS Code 窗口和相关终端后，可将外层目录重命名为 `电气工程及其自动化+2307+202310080241+王子成+光伏电站日前功率预测与运行工作台`。
...
- 更新 `docs/index.html`：
  - 将 GitHub Pages 首屏改成“光伏电站日前功率预测与运行工作台”，核心结果卡改为历史功率基线、气象预报融合、局地校正融合。
